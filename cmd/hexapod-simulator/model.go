...
func NewModel(pose *pose.Body) *Model {
	...
	model.Shader.UpdateLocation(rl.LocMatrixModel, rl.GetShaderLocation(model.Shader, "mMatrix"))
	model.Shader.UpdateLocation(rl.LocMatrixView, rl.GetShaderLocation(model.Shader, "view"))
	model.Shader.UpdateLocation(rl.LocVectorView, rl.GetShaderLocation(model.Shader, "viewPos"))
...
func (model *Model) Draw() {
	...
	for _, leg := range model.Pose.Legs() {
		...
		labelPosition := rl.Vector3Transform(rl.Vector3{}, transform)
		...
		for _, hinge := range leg.Hinges() {
			...
			hingeCenter := rl.Vector3Transform(rl.Vector3{}, transform)
		...
		effectorWorldSpace := rl.Vector3Transform(rl.Vector3{}, transform)
